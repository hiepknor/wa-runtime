...
export class CampaignRunRepository {
  ...
  async applyPreflight(runId: string, report: CampaignPreflightDto): Promise<void> {
    await this.database.transaction(async client => {
      ...
      if (report.status === 'BLOCK') {
        await client.query(
          `UPDATE campaign_runs SET status = 'BLOCKED', preflight_status = $2,
      ...
      await client.query(
        `UPDATE campaign_runs SET status = $2::campaign_run_status,
  ...
  async markPreparationFailed(runId: string): Promise<void> {
    await this.database.query(
      `UPDATE campaign_runs SET status = 'FAILED', completed_at = now(), updated_at = now()
  ...
  async activateDueRuns(): Promise<number> {
    const result = await this.database.query(
      `UPDATE campaign_runs SET status = 'RUNNING', started_at = COALESCE(started_at, now()), updated_at = now()
  ...
  async materializePending(runId: string, maxBuffered: number): Promise<number> {
    return this.database.transaction(async client => {
      const runResult = await client.query<{
        ...
        payload_snapshot: { text: string };
        status: string;
      }>('SELECT session_id, execution_mode, payload_snapshot, status FROM campaign_runs WHERE id = $1 FOR UPDATE', [runId]);
